...
> Last updated: 2026-08-03 | PR MERGED: #19–#52, #54–#65, #67, #69–#71, #73, #75–#77, #79–#80 (#68, #74, #78 closed/superseded) — lihat tabel pointer di bawah. PR #79 (mobile reorder + keyboard-safe notes + notes clamp) merged & diverifikasi langsung di produksi 2026-08-02 (album baru + upload foto + full lifecycle test). PR #81–#87: audit penuh codebase (2026-08-02) dikerjakan 7 subagent paralel di 7 branch terpisah. Direview independen (2 REQUEST CHANGES, 5 APPROVE WITH NITS), dibandingkan dengan bot review (cubic-dev-ai/CodeRabbit/Graphite/Junie Review), lalu di-follow-up penuh 2026-08-03 (commit baru di tiap branch, termasuk 1 bug tersembunyi baru di `albumDeletion.ts` yang ditemukan sendiri). **Round ke-2 follow-up 2026-08-03 selesai: seluruh temuan bot yang valid sudah difix (commit baru per PR), semua thread review bot di GitHub sudah dibalas+di-resolve, 0 thread unresolved tersisa di ketujuh PR; semua CI SUCCESS + Vercel mergeable.** Semua 7 masih **OPEN**, menunggu keputusan merge. Full narratives: `~/.junie/tasks/`.
...
| #81 | Audit fix: `scripts/upload.py` — critical `NameError` (client.upload), watchdog import crash, thread-safety, validate-before-hash, asset `_id` ref, hapus `--token` CLI | OPEN | `~/.junie/tasks/AUDIT-2026-08-02-fixes-PR81-87.md` |
| #82 | Audit fix: lib session/cache hardening — `auth.ts` fail-closed, Upstash fetch timeout, cache stampede hard-miss dedup, EXIF selalu di-strip, worker idle-terminate, realtime hook `.catch()` | OPEN | `~/.junie/tasks/AUDIT-2026-08-02-fixes-PR81-87.md` |
| #83 | Audit fix: Sanity PIN/session security — token read/write terpisah, Studio env vars, `pin` keluar dari query yang di-cache, `gallerySession` diikat PIN hash | OPEN | `~/.junie/tasks/AUDIT-2026-08-02-fixes-PR81-87.md` |
| #84 | Audit fix: `GalleryPage` — pesan error server asli (429/403/404), updater `setSelectedPhotos` impure diperbaiki, guard submit ganda, URL galeri di-paste tidak di-slugify karakter demi karakter | OPEN | `~/.junie/tasks/AUDIT-2026-08-02-fixes-PR81-87.md` |
| #85 | Audit fix: CI/config hardening — timeout+concurrency workflow, pin action ke SHA, dependency hygiene (`package.json` dev/prod split), `check-hydration-leak.mjs` per-tag parse, `vercel.json` region+header | OPEN | `~/.junie/tasks/AUDIT-2026-08-02-fixes-PR81-87.md` |
| #86 | Audit fix: API validation — body type/format checks (albums), urutan cache-invalidate sebelum publish event (finalize), `submit.ts` gate sesi sebelum status, `request.json()` try/catch di 7 route | OPEN | `~/.junie/tasks/AUDIT-2026-08-02-fixes-PR81-87.md` |
| #87 | Audit fix: admin UI bugs — `AlbumFormModal` reset effect, `formatDate` timezone off-by-one, `AdminPage` remount berlebihan, `UploadPage` upload-safety, a11y tablist→button | OPEN | `~/.junie/tasks/AUDIT-2026-08-02-fixes-PR81-87.md` |
