...
| #110 | Fix: `eventDate` di form album — larangan tanggal lampau dihapus (client `DatePickerField` min + validasi server) karena album biasa dibuat setelah acara berlangsung; follow-up: `eventDate` dibuat opsional penuh (form, create/update API, schema Sanity `Rule.required()` dihapus, clear-ke-kosong via `.unset()` pola sama dgn `customSlug`) | OPEN — 276/276 vitest (+9 test baru `validateAlbumForm`), tsc/eslint bersih, diverifikasi live lokal (create tanpa tanggal + create dgn tanggal lampau, keduanya sukses); CI GitHub sedang berjalan | `~/.junie/tasks/PR-110-album-event-date.md` |
