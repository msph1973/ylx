...
> Last updated: 2026-09-02 | PR MERGED: #19–#52, #54–#65, #67, #69–#71, #73, #75–#77, #79–#108 (#68, #74, #78 closed/superseded) — PR #109 OPEN — lihat tabel pointer di bawah. **PR #105 MERGED** (squash `0f07442`, 2026-08-31) — toast sukses submit; unlock tidak lagi hapus pilihan client (cuma reopen), reset destruktif dipisah ke endpoint/tombol/event realtime sendiri (`album:reset`); galeri pre-fill pilihan tersimpan dari server; 267/267 vitest; live browser E2E di preview PASSED sebelum merge (unlock-preserves, submit toast 200, reset clears + delivered-reset 409). **PR #106 MERGED** (squash `6ca3358`, 2026-08-31) — motion press-feedback + spinner progress untuk submit/lock/unlock/reset + badge-exit diperbaiki (`AnimatePresence`), konstanta bersama `src/lib/motion.ts`; 267/267 vitest; live DOM-measured di preview PASSED; 3 ronde Kilo bersih. **PR #108 MERGED** (squash `392a8f1`, 2026-09-02) — fix font "Enter PIN" kegedean + posisi PIN card kejauhan ke bawah di mobile (duplikat `100vh` + `.gallery-layout` flex→grid); album title tampil lagi di atas kartu PIN + header post-auth pakai clientName asli; ronde 2 (cubic) fix clipping PIN card di viewport pendek + reset placeholder saat sesi expired — diverifikasi Kernel+agent-browser di preview sebelum merge. **PR #107 MERGED** (squash `b006c0f`, 2026-09-02) — polish UI/UX copy/PIN-card/lightbox/admin hierarchy; sempat conflict vs master setelah #108 merge duluan (keduanya independen nambah fitur album title di atas kartu PIN), diresolve dgn merge master masuk branch #107 (pilih versi #108 yg sudah merged); 267/267 vitest, semua CI SUCCESS sebelum merge. **PR #109 OPEN** — redesign total homepage (`/impeccable craft`, branch `redesign/homepage-editorial`): wordmark besar + tekstur grain/glow (tanpa foto, keputusan user), panel akses galeri underline-style, admin link dipindah ke pojok, semua copy Bahasa Indonesia; token warna/font existing dipertahankan (bukan palet baru); tsc/eslint/vitest (267/267)/astro build semua bersih, semua CI check SUCCESS, bot review cubic/Mesrai masih pending — menunggu review user sebelum merge.
...
| #109 | Redesign total homepage (`/impeccable craft`): wordmark besar + tekstur grain/glow (tanpa foto), panel akses galeri underline-style, admin link dipindah ke pojok, semua copy Bahasa Indonesia; token warna/font existing dipertahankan | OPEN — semua CI check SUCCESS, bot review cubic/Mesrai masih pending, menunggu review user | `~/.junie/tasks/PR-109-homepage-redesign.md` |
