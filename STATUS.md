...
| #108 | Fix: font "Enter PIN" kegedean + PIN card kejauhan ke bawah di mobile — hapus duplikat `min-height:100vh` di `.gallery-auth`, `.gallery-layout` diganti flex→grid (`auto 1fr`) supaya `height:100%` genuinely resolve (flexbug), title `text-2xl` di mobile; follow-up: album title kini tampil lagi di atas kartu PIN (`albumTitle` prop) + header post-auth tampilkan `clientName` asli (ganti placeholder statis "Client" via DOM effect, karena header itu markup Astro di luar island React) | OPEN — verified Kernel+agent-browser iPhone 15 & 375x667 + album title/clientName di preview | `~/.junie/tasks/PR-108-pin-entry-mobile-layout.md` |
