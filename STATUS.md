...
> Last updated: 2026-09-01 | PR MERGED: #19–#52, #54–#65, #67, #69–#71, #73, #75–#77, #79–#106 (#68, #74, #78 closed/superseded) — lihat tabel pointer di bawah. **PR #105 MERGED** (squash `0f07442`, 2026-08-31) — toast sukses submit; unlock tidak lagi hapus pilihan client (cuma reopen), reset destruktif dipisah ke endpoint/tombol/event realtime sendiri (`album:reset`); galeri pre-fill pilihan tersimpan dari server; 267/267 vitest; live browser E2E di preview PASSED sebelum merge (unlock-preserves, submit toast 200, reset clears + delivered-reset 409). **PR #106 MERGED** (squash `6ca3358`, 2026-08-31) — motion press-feedback + spinner progress untuk submit/lock/unlock/reset + badge-exit diperbaiki (`AnimatePresence`), konstanta bersama `src/lib/motion.ts`; 267/267 vitest; live DOM-measured di preview PASSED; 3 ronde Kilo bersih. **PR #107 OPEN** — polish UI/UX copy/PIN-card/lightbox/admin hierarchy, 2 saran Kilo sudah diinvestigasi & terjawab (bukan bug). **PR #108 OPEN** (`fix/pin-entry-mobile-layout`, 2026-09-01) — fix font "Enter PIN" kegedean + posisi PIN card kejauhan ke bawah di mobile (duplikat `100vh` di dalam `.gallery-content` + `.gallery-layout` pakai flex bukan grid, keduanya bikin `height:100%` gak resolve akurat); diverifikasi via Kernel cloud browser + agent-browser pada viewport iPhone 15 (390x844) & 375x667 di preview — tidak overflow, card center rapi, font 24px di mobile. Follow-up sama PR: album title tampil lagi di atas kartu PIN + header post-auth pakai clientName asli (bukan placeholder "Client"); ronde 2 (cubic review) fix `.gallery-auth` height->min-height (cegah PIN card kepotong di viewport pendek) + header client-name direset ke placeholder saat sesi expired (cegah nama client lama bocor) — semua diverifikasi live di preview.
