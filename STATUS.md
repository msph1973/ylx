...
> Last updated: 2026-09-13 | PR MERGED: #19–#52, #54–#65, #67, #69–#71, #73, #75–#77, #79–#116 (#68, #74, #78 closed/superseded) — tidak ada PR terbuka — lihat tabel pointer di bawah. **PR #113 MERGED** (squash `418d801`, 2026-09-13) — halaman statis /privacy + /terms untuk syarat publish OAuth Google (Authorized domains + link privacy/ToS); 8 temuan bot ditangani (5 Qodo diperbaiki/dibalas-berbukti, 3 Kilo diperbaiki); semua CI SUCCESS. **PR #111 MERGED** (squash `111ea95`, 2026-09-13) — polish frontend (LazyMotion admin, kontrol 44px, dvh, lightbox single-res + fetchPriority first-row); Qodo 5 temuan fixed, semua CI SUCCESS. **PR #112 MERGED** (squash `643f802`, 2026-09-13) — S2 multitenant (superadmin/vendor + migrasi role, invite-only Google login + tombol GIS, isolasi owner semua endpoint admin, realtime per-owner, brand per akun, vendor Drive-only, profil vendor isi-nama-sendiri); CodeQL ReDoS + 9 Qodo + Kilo ditangani, semua CI SUCCESS (finally via rebase di atas #111), Steel live OK termasuk login Google vendor perdana. **PR #114 MERGED** (squash `48a22c8`, 2026-09-13) — fix mobile vendor (Drive row stack + toolbar compact).angsung; follow-up: `eventDate` dibuat opsional penuh (form, create/update API, schema Sanity `Rule.required()` dihapus, clear-ke-kosong via `.unset()` pola sama dgn `customSlug`); 276/276 vitest (+9 test baru), tsc/eslint bersih, semua CI SUCCESS sebelum merge. **PR #108 MERGED** (squash `392a8f1`, 2026-09-02) — fix font "Enter PIN" kegedean + posisi PIN card kejauhan ke bawah di mobile (duplikat `100vh` + `.gallery-layout` flex→grid); album title tampil lagi di atas kartu PIN + header post-auth pakai clientName asli; ronde 2 (cubic) fix clipping PIN card di viewport pendek + reset placeholder saat sesi expired — diverifikasi Kernel+agent-browser di preview sebelum merge. **PR #107 MERGED** (squash `b006c0f`, 2026-09-02) — polish UI/UX copy/PIN-card/lightbox/admin hierarchy; sempat conflict vs master setelah #108 merge duluan (keduanya independen nambah fitur album title di atas kartu PIN), diresolve dgn merge master masuk branch #107 (pilih versi #108 yg sudah merged); 267/267 vitest, semua CI SUCCESS sebelum merge. **PR #109 MERGED** (squash `c99204b`, 2026-09-03) — redesign total homepage (`/impeccable craft`, branch `redesign/homepage-editorial`): wordmark besar + tekstur grain/glow (tanpa foto, keputusan user), panel akses galeri underline-style, admin link dipindah ke pojok, semua copy Bahasa Indonesia, plus follow-up animasi (blur-to-focus entrance, idle glow breathing, cursor-reactive glow parallax, edition tag, focus micro-interaction); token warna/font existing dipertahankan (bukan palet baru); tsc/eslint/vitest (267/267)/astro build semua bersih, semua CI check SUCCESS sebelum merge.

Baca file ini pertama kali sebelum file lain. Ini adalah satu-satunya sumber kebenaran tentang kondisi project saat ini.

> 📋 **Fitur masa depan yang sudah disetujui user tapi belum dikerjakan** (backup Sanity, watermark, tingkatan pilihan, load-testing, multi-admin ownership, branding kustom, dashboard analitik, i18n — notifikasi email & final delivery/download foto sudah **MERGED**, lihat header di atas) — lihat `ROADMAP.md` di root project untuk detail teknis lengkap per item sebelum mulai mengerjakan salah satunya.
...
| #88 | Feat: kalender date-picker kustom untuk Event Date di AlbumFormModal (ganti `<input type="date">` native, larangan tanggal lampau tetap dipertahankan + kini terlihat visual di grid; 2026-08-04: 4 ronde follow-up bot review (state pure, popover flip+clipping fix, focusout close, arrow-key APG nav, aria-pressed; 5 nit form-parity Junie di-reject terverifikasi), CI 12/12 hijau, semua thread resolved, diverifikasi live E2E di preview, merged `1d85369`) | MERGED | `~/.junie/tasks/PR-88-calendar-date-picker.md` |
| #89 | Perf audit: paralelisasi 6 lokasi fetch Sanity independen (`Promise.all`) di `draft.ts`, `admin/albums.ts`, `admin/albums/[id]`, `admin/photos/[id]`, `admin/photos/bulk-delete.ts`, `admin/upload/finalize.ts`; sengaja tidak menyentuh `verify.ts`/urutan invalidate-lalu-publish (regresi `waitUntil` sebelumnya) | MERGED | `~/.junie/tasks/AUDIT-PERF-2026-08-06-PR89-92.md` |
| #90 | Perf audit: migrasi `framer-motion` ke `LazyMotion`+`domAnimation`+`m` di `GalleryPage`/`PhotoLightbox`/`PinEntry` (bundle chunk animasi 112.3K→72.3K raw, -36%; gzip -30%); fix follow-up: hapus provider duplikat + `shouldReduceMotion` di transisi PIN error/loading | MERGED | `~/.junie/tasks/AUDIT-PERF-2026-08-06-PR89-92.md` |
| #91 | Perf audit: memoisasi `PhotoTile` (AlbumDetail admin) + `GalleryPhotoTile` (galeri) dgn `React.memo`+comparator eksplisit agar drag/hover cuma re-render tile terpengaruh; hoist CSS besar (`GALLERY_VIEW_STYLES`, `ALBUM_CARD_STYLES`) & `useCallback`/`useMemo` variants ke luar render | MERGED | `~/.junie/tasks/AUDIT-PERF-2026-08-06-PR89-92.md` |
| #92 | Fix CI: scope `concurrency` group `junie-review.yml` per `github.event_name` — event `issue_comment` (nyaris selalu no-op) sempat ikut membatalkan job review `pull_request` yang sedang berjalan karena berbagi grup yang sama | MERGED | `~/.junie/tasks/AUDIT-PERF-2026-08-06-PR89-92.md` |
| #94 | Feat: integrasi Sentry error monitoring opt-in (`@sentry/astro`, hanya aktif kalau `PUBLIC_SENTRY_DSN` di-set) — `captureError()` di 29/31 lokasi `console.error` endpoint API, session replay + browser tracing di client, environment tagging via `VERCEL_ENV` di server, source-map upload opsional; `robots.txt` disallow semua; fix follow-up bot review (dedupe capture ganda ke Sentry, hapus dead-code capture di `releaseSlugLock` call-site → dipindah ke dalam helper itu sendiri, skip panggilan SDK penuh kalau DSN tak di-set agar tak ada noise warning) | MERGED | `~/.junie/tasks/PRODUCTION-READINESS-2026-08-10.md` |
...
| #105 | Fix: toast sukses saat client submit pilihan (sebelumnya cuma error path yang ada feedback); `unlock.ts` tidak lagi hapus `selection`/`submission` — cuma buka status jadi `active` (pilihan client tetap ada utk direvisi); endpoint baru `POST admin/albums/[id]/reset` (logic hapus lama) + tombol admin "Reset Selection" terpisah dari "Unlock Gallery", event realtime `album:reset` terpisah dari `album:unlocked`; galeri client kini pre-fill pilihan tersimpan dari server (`albumBySlugQuery` bawa `selections`) di setiap load/unlock, bukan cuma draft lokal | MERGED (`0f07442`) | `~/.junie/tasks/PR-105-submit-notice-preserve-selections.md` |
| #106 | Enhance: motion — press feedback (whileTap/:active scale) di puck select, submit, tombol admin; spinner progress in-flight ("Submitting…/Unlocking…/Resetting…/Mengunduh…") di tombol + ConfirmDialog; badge seleksi tile exit-nya diperbaiki (dulu dead code, tak ada AnimatePresence); `src/lib/motion.ts` bersama; reduced-motion tetap dihormati | MERGED (`6ca3358`) — live motion check PASSED; 3 ronde review Kilo ditangani (head `8b022e5`), cubic+Kilo clean, CI hijau | `~/.junie/tasks/PR-106-motion-click-progress.md` |
| #107 | Polish UI/UX: copy EN seragam (Final/Originals/photos selected/Downloading), identitas album di kartu PIN, lightbox Select primary, admin PIN masked + baris Status hapus + banner Drive lembut + pager 1-halaman hidden | MERGED (squash `b006c0f`, 2026-09-02) — sempat conflict vs master setelah #108 (dua fitur album-title-di-PIN independen), diresolve pilih versi #108; 267/267 vitest, CI SUCCESS | `~/.junie/tasks/PR-107-ui-polish.md` |
| #108 | Fix: font "Enter PIN" kegedean + PIN card kejauhan ke bawah di mobile — hapus duplikat `min-height:100vh` di `.gallery-auth`, `.gallery-layout` diganti flex→grid (`auto 1fr`) supaya `height:100%` genuinely resolve (flexbug), title `text-2xl` di mobile; follow-up 1: album title kini tampil lagi di atas kartu PIN (`albumTitle` prop) + header post-auth tampilkan `clientName` asli (ganti placeholder statis "Client" via DOM effect, karena header itu markup Astro di luar island React); follow-up 2 (cubic review): `.gallery-auth` height->min-height (cegah PIN card kepotong di viewport pendek/zoom), header client-name direset ke "Client" saat isAuthenticated false (cegah nama client lama bocor saat sesi expired) | MERGED (squash `392a8f1`, 2026-09-02) — verified Kernel+agent-browser iPhone 15 & 375x667 + album title/clientName + cubic re-review "All reported issues were addressed" sebelum merge | `~/.junie/tasks/PR-108-pin-entry-mobile-layout.md` |

| #109 | Redesign total homepage (`/impeccable craft`): wordmark besar + tekstur grain/glow (tanpa foto), panel akses galeri underline-style, admin link dipindah ke pojok, semua copy Bahasa Indonesia; token warna/font existing dipertahankan; follow-up: choreographed hero entrance (blur-to-focus wordmark, staggered delays), idle glow breathing, cursor-reactive glow parallax, magazine edition tag, input focus micro-interaction — no new deps | MERGED (squash `c99204b`, 2026-09-03) — 267/267 vitest, semua CI SUCCESS, diverifikasi live di Vercel Preview via Kernel+agent-browser (iPhone 15) sebelum merge | `~/.junie/tasks/PR-109-homepage-redesign.md` |
| #110 | Fix: `eventDate` di form album — larangan tanggal lampau dihapus (client `DatePickerField` min + validasi server) karena album biasa dibuat setelah acara berlangsung; follow-up: `eventDate` dibuat opsional penuh (form, create/update API, schema Sanity `Rule.required()` dihapus, clear-ke-kosong via `.unset()` pola sama dgn `customSlug`) | MERGED (squash `9bdd38d`, 2026-09-03) — 276/276 vitest (+9 test baru `validateAlbumForm`), tsc/eslint bersih, semua CI SUCCESS, diverifikasi live lokal (create tanpa tanggal + create dgn tanggal lampau, keduanya sukses) | `~/.junie/tasks/PR-110-album-event-date.md` |
| #113 | Feat: halaman statis /privacy + /terms (syarat publish OAuth Google) — di-split dari branch S2 agar produksi serve tanpa tunggu merge S2; follow-up: policy diperbanyak (sub-prosesor, scope Drive SA, metadata Ably, Sentry replay), bullets + mailto | MERGED (squash `418d801`, 2026-09-13) — live 200 di ylex.my.id, 8 temuan bot resolved, semua CI SUCCESS | `~/.junie/tasks/PR-113-oauth-legal-pages.md` |
| #111 | Polish frontend S1 (LazyMotion admin + `m` descendants, kontrol sentuh 44px, dvh, lightbox single-res tanpa srcSet, fetchPriority high 2 tile pertama, sizes 36px) — branch `polish/frontend-quick-wins`, Qodo 5 temuan fixed (crop, layout anim, motion descendants, thumbnails, priority), Kilo No Issues | MERGED (squash `111ea95`, 2026-09-13) — semua CI SUCCESS | `~/.junie/tasks/PR-111-frontend-polish-s1.md` |
| `~/.junie/tasks/PR-112-s2-multitenant.md` |
| #114 | Fix mobile vendor: baris link Drive di modal menumpuk vertikal <480px (sebelumnya input menciut vs tombol 100%), toolbar admin ellipsis + tombol kompak nowrap (fix urutan cascade media query temuan Qodo) | MERGED (squash `48a22c8`, 2026-09-13) — Steel 402x874 before/after + computed-style proof, semua CI SUCCESS | `~/.junie/tasks/PR-114-vendor-mobile-polish.md` |
| #115 | Docs/skills consistency P0: STATUS/AGENTS/ROADMAP/cubic/REVIEW/.env disinkron ke S2, `.letta/` dihapus (Junie sole driver), skill audit+monitor ditulis ulang (redaksi password, Steel ganti Kernel); ronde 2 (9 temuan bot valid) | MERGED (squash `35b8a7c`, 2026-09-13) — semua CI SUCCESS, Steel produksi OK (login Google live, galeri 325/high=2, dashboard vendor) | `~/.junie/tasks/PR-115-docs-consistency-p0.md` |
| #116 | Opsi 3 (small debt): aksen `#c4704b` dimenangkan (5.44:1 AA) + DESIGN/PRODUCT sinkron (termasuk token ring), `seed-superadmin.mjs` bootstrap (prompt tersembunyi, sentinel atomik, cap 72B, teruji live), `.env.example` override skrip | MERGED (squash `182f5e5`, 2026-09-13) — 12 threads (3 valid + 1 bug import nyata), Steel preview OK, produksi 200 | `~/.junie/tasks/PR-116-small-debt-batch.md` |
