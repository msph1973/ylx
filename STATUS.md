...
| #108 | Fix: font "Enter PIN" kegedean + PIN card kejauhan ke bawah di mobile — hapus duplikat `min-height:100vh` di `.gallery-auth`, `.gallery-layout` diganti flex→grid (`auto 1fr`) supaya `height:100%` genuinely resolve (flexbug), title `text-2xl` di mobile | OPEN — verified Kernel+agent-browser iPhone 15 & 375x667 di preview | `~/.junie/tasks/PR-108-pin-entry-mobile-layout.md` |
