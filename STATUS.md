...
> Last updated: 2026-08-15 | PR MERGED: #19–#52, #54–#65, #67, #69–#71, #73, #75–#77, #79–#96 (#68, #74, #78 closed/superseded) — lihat tabel pointer di bawah. **PR #96 (final delivery) MERGED** (merge commit `2178cc5`, 2026-08-15) — admin upload foto final ke album, status `delivered`, galeri klien tampil foto final otomatis dengan realtime sync + idempotent upload, opsi admin "beri akses foto original" (`showOriginalAfterDelivery`) + tab "Cetak"/"Semua Foto" + download per-foto & download-all `.zip` (client-side, `jszip`) di galeri klien setelah delivered. Sebelum merge, 2 ronde tambahan bot-review pasca push terakhir ditutup: dedupe helper `delay()` (reuse `sanityUpload.ts`), guard `deleteOrphanedAsset` supaya tidak menghapus asset Sanity yang ternyata masih direferensikan dokumen lain (dedup by content-hash), dan `jszip` dipindah ke dynamic import supaya tidak membengkakkan bundle `client:load` galeri. PR #79 (mobile reorder + keyboard-safe notes + notes clamp) merged & diverifikasi langsung di produksi 2026-08-02 (album baru + upload foto + full lifecycle test). PR #81–#87: audit penuh codebase (2026-08-02) dikerjakan 7 subagent paralel di 7 branch terpisah, direview independen, dibandingkan dengan bot review, di-follow-up 2 ronde — **SEMUA 7 PR sudah MERGED** (satu konflik nyata diselesaikan manual — `submit.ts`/`admin/albums.ts`, PR #86 vs PR #83). **PR #88 MERGED**: kalender date-picker kustom untuk Event Date di `AlbumFormModal`. **Audit performa — PR #89–#92 semua MERGED** (3 subagent paralel + 1 fix CI tambahan): parallelize fetch Sanity, LazyMotion bundle shrink, memoisasi re-render, fix concurrency group CI. **2026-08-10: PR #94 MERGED** — integrasi Sentry error monitoring (opt-in, hanya aktif kalau `PUBLIC_SENTRY_DSN` di-set; `captureError` di 29 endpoint API, session replay + tracing di client, source-map upload opsional) + `robots.txt` (disallow semua — tidak ada konten publik yang perlu diindeks) + housekeeping (tutup issue #93 false-positive, hapus env var `MASTRA_API_KEY` menganggur). 4 env var Sentry (`PUBLIC_SENTRY_DSN`/`SENTRY_PROJECT`/`SENTRY_ORG`/`SENTRY_AUTH_TOKEN`) di-set **Production-only** (keputusan eksplisit user, bukan Preview — auth token org tidak perlu ikut ke URL preview publik). Vercel Production deployment untuk commit merge (`6c73b4d`) **success** (verified 200 di homepage/admin-login/robots.txt). ROADMAP.md #11 (download foto) juga **MERGED** bersama PR #96 (klien-side, menggantikan keputusan admin-only lama, dikonfirmasi user 2026-08-15). Detail: `~/.junie/tasks/FEATURE-final-delivery.md`. Full narratives: `~/.junie/tasks/`.
