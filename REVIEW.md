...
| `GDRIVE_CLIENT_EMAIL` | Drive-storage albums — service account for folder scans (`lib/gdrive.ts`, called from `api/admin/albums/scan-drive.ts`) | ❌ optional — when unset, the scan endpoint 500s with "not configured" and the Drive storage option is effectively disabled; set in **both** Production + Preview when enabled |
