...
- **Session end (every task that changes the project or its state):** append one short dated entry `## [YYYY-MM-DDTHH:MMZ] <task>` to `~/.junie/memory/notes.md` (1-3 bullets: what changed, PR/commit, pointer to the relevant `~/.junie/tasks/PR-<n>-<slug>.md` for full detail — not a copy of it) and sync `STATUS.md`'s pointer table.
...
### Task narratives (`~/.junie/tasks/`, local-only, never pushed)

Full narrative for each task/PR (root cause, code changes, bot-review rounds, verification) lives in `~/.junie/tasks/PR-<n>-<slug>.md` — **one file per task**, outside the project repo entirely, so it's never git-tracked/pushed and never shows up in a PR diff/review. See `~/.junie/tasks/README.md` for the exact convention. This directory is distinct from `~/.junie/memory/tasks/`, a different pre-existing directory auto-generated by another coding agent's own pipeline that also operates on this repo — do not confuse the two, and do not write Junie's task narratives into that other directory.

- **While working on a task:** create/update its file in `~/.junie/tasks/` as work progresses (status `IN PROGRESS`) — this is where the detailed narrative belongs, not `notes.md`/`STATUS.md`.
- **On completion/merge:** mark the file `MERGED`/`CLOSED`/`SUPERSEDED`; never delete it — it's the durable local record.
- `STATUS.md` only keeps a **lean pointer table** (PR/title/status/file) plus a 1-line header summary — never the full narrative.
- `docs/history/STATUS-ARCHIVE.md` is **frozen** (covers PR #19–#40 only, per its own header note) — do not add new entries there.

### Memory MCP knowledge graph (structured facts, not a markdown duplicate)

The built-in `memory` MCP server holds a knowledge graph — use it **only** for structured facts that are hard to keep track of in flat markdown (architecture decisions with a non-obvious "why", tooling gotchas, cross-PR dependencies). It must **not** duplicate the narrative already in `STATUS.md`/`docs/history/`/`~/.junie/tasks/`.
