...
- **Bumps:** `astro` `^5.18.2` → `^6.4.8`; `@astrojs/vercel` `^8.2.11` → `^10.0.8` (peer `astro@^6`); `@astrojs/react` `^4.4.2` → `^5.0.7` (engine Node `>=22.12`, peer React 18 tetap OK); root `engines.node` `>=20` → `>=22` (Astro 6 minimum Node 22).
...
- **Verifikasi:** `astro sync` + `tsc --noEmit` lolos, `eslint --max-warnings 0` lolos, `vitest` 3/3, `playwright` 12 pass + 1 flaky (tes paginasi lama, lolos saat retry), `pnpm build` lolos (adapter Vercel v10).
